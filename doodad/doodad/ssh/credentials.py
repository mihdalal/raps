...
class SSHCredentials(object):
    ...
    def __init__(self, hostname=None, username=None, password=None, identity_file=None):
        assert (
            password is not None or identity_file is not None
        ), "One of password or identity_file must be provided"
    ...
    def get_ssh_cmd_prefix(self):
        ...
        if self.identity_file:
            ...
            print(
                "WARNING: Using password-based ssh is not secure! Please consider using identity files."
            )
        else:
