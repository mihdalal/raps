...
class MountLocal(Mount):
    ...
    def gzip(self):
        ...
        def filter_func(tar_info):
            filt = any([tar_info.name.endswith(ext) for ext in self.filter_ext]) or any(
                [tar_info.name.endswith("/" + ext) for ext in self.filter_dir]
            )
            if filt:
                return None
            return tar_info
        ...
        with tempfile.NamedTemporaryFile("wb+", suffix=".tar") as tf:
            ...
            with tarfile.open(fileobj=tf, mode="w") as tar:
                ...
                tar.add(
                    self.local_dir,
                    arcname=os.path.basename(self.local_dir),
                    filter=filter_func,
                )
            tf.seek(0)
            yield tf.name
